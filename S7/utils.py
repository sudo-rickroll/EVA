class DataProcessing():
  ...
  def Dataset(self, root, train, download=True, *Transforms):    
    if train == True:
      for item in Transforms:
        if str(item) not in list(map(str, self.Transforms_Train)) : self.Transforms_Train.append(item)
      return getattr(torchvision.datasets,self.dataset)(root=root, train=train, download=download, transform=transforms.Compose(self.Transforms_Train))
    else:
      for item in Transforms:
        if str(item) not in list(map(str, self.Transforms_Test)) : self.Transforms_Test.append(item)
      return getattr(torchvision.datasets,self.dataset)(root=root, train=train, download=download, transform=transforms.Compose(self.Transforms_Test))
